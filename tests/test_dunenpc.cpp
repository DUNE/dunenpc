...
int main()
{
  ...
  if (createRandomPayload(my_local_url) == 1) return 1;
  resp = client.insertPayload("my_pt", my_local_url, iov);
  std::cout << resp << std::endl;
  if (resp["code"] == 0) return 1;
  ...
  resp = client.insertPayload("my_pt", my_local_url, iov_end);
  std::cout << resp << std::endl;
  if (resp["code"] != 0) return 1;
  ...
  resp = client.insertPayload("my_pt", my_local_url, 0);
  std::cout<<resp<<std::endl;
  if (resp["code"]==0) return 1;
  client.setGlobalTag("my_gt");

  // ... same for payload type that does not exist...
  resp = client.insertPayload("non_existing_type", my_local_url, 0);
  std::cout<<resp<<std::endl;
  if (resp["code"]==0) return 1;

  // ... and if the payload file does not exist locally...
  resp = client.insertPayload("my_pt", "non_existing_file", 0);
  std::cout<<resp<<std::endl;
  if (resp["code"]==0) return 1;
